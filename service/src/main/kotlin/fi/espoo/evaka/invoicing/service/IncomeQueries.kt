...
fun Database.Read.expiringIncomes(
    ...
    WHERE receiver_id = pl.person_id AND notification_type = :notificationType 
...
WITH latest_income AS (
    ...
    SELECT DISTINCT i.id, i.person_id, i.valid_to
    FROM placement pl
...
WHERE NOT EXISTS (
    ...
    WHERE person_id = pl.person_id AND created > :today - INTERVAL '1 month'
